...
function repo() {
  ...
  return sanitizeHeaderValue(r);
}

function branch() {
  return sanitizeHeaderValue(process.env.GITHUB_BRANCH || 'main') || 'main';
}

/**
 * Strip any character HTTP header values can't carry (must be Latin-1).
 * Copy/paste from rich-text sources sometimes injects invisible separators
 * like U+2028 / U+2029 / non-breaking spaces that would otherwise crash the
 * fetch with "Cannot convert argument to a ByteString".
 */
...
function headers() {
  ...
  if (!raw) throw new Error('GITHUB_TOKEN env var not configured');
  const token = sanitizeHeaderValue(raw);
  if (!token) throw new Error('GITHUB_TOKEN is empty after sanitisation');
  return {
...
export async function putFile(path, content, message, sha = null) {
  ...
  if (!res.ok) {
    ...
    // GitHub returns 404 on write when the PAT lacks Contents: Write,
    // to avoid leaking which repos a token can read. Surface that.
    if (res.status === 404) {
      throw new Error(
        `GitHub putFile 404: repo or write permission missing. Check that GITHUB_REPO (${repo()}) is correct and the fine-grained PAT has "Contents: Read and write" on this repo. Raw: ${body}`
      );
    }
    throw new Error(`GitHub putFile ${res.status}: ${body}`);
  }
  return res.json();
}
