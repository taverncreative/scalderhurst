...
export function buildPostFile(input, { mode = 'create', existingFilename = null } = {}) {
  ...
  // Accept canonical category slugs OR any custom string the editor types
  // in via the "Other" option. We normalise to a slug for storage; the build
  // pipeline will title-case it for display on the archive.
  const categoryRaw = category == null ? '' : String(category).trim();
  if (!categoryRaw) return { ok: false, error: 'Category is required' };
  const categorySlug = CATEGORIES.has(categoryRaw) ? categoryRaw : slugify(categoryRaw);
  if (!categorySlug) return { ok: false, error: 'Category must contain at least one letter or digit' };
  if (!date) return { ok: false, error: 'Publish date is required' };
  if (isNaN(new Date(date).getTime())) return { ok: false, error: 'Invalid date' };
  // Cover image is optional — posts without one render as text-only.
  // If a cover IS provided we still want alt text for accessibility.
  const hasCover = cover && String(cover).trim();
  if (hasCover && (!cover_alt || !String(cover_alt).trim())) {
    return { ok: false, error: 'Please add alt text for the cover image (accessibility).' };
  }
