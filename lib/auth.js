...
/**
 * Where the canonical (post-bootstrap) password hash lives — in the GitHub
 * repo, read at runtime via the Contents API (lib/github.js), never from the
 * deployed filesystem.
 * - On first deploy this file doesn't exist; we fall back to ADMIN_PASSWORD_HASH.
 * - After the customer changes their password via /api/change-password, the
 *   new hash is committed here and used in preference to the env var.
 * - scripts/prune-private.mjs deletes content/.admin/ from the Vercel build
 *   output so the file is never deployed as a static asset. (Rewrites cannot
 *   block it: Vercel serves existing files before evaluating rewrites.)
 */
export const PASSWORD_HASH_PATH = 'content/.admin/password-hash.txt';
...
/**
 * Resolve the current admin password hash.
 * - Tries the repo file first (canonical after any password change).
 * - Falls back to the ADMIN_PASSWORD_HASH env var (initial bootstrap value).
 * - Returns null if neither is available.
 *
 * Note: This makes ONE GitHub API round-trip per login. Acceptable: logins
 * are rare, and the function gracefully falls back to the env var on any
 * network/permission error.
 */
export async function getCurrentPasswordHash() {
  try {
    const { getFile } = await import('./github.js');
    const file = await getFile(PASSWORD_HASH_PATH);
    if (file && file.content && file.content.trim()) {
      return { hash: file.content.trim(), sha: file.sha, source: 'file' };
    }
  } catch (err) {
    console.warn('[auth] could not read stored hash, falling back to env:', err.message);
  }
  const envHash = process.env.ADMIN_PASSWORD_HASH;
  return envHash ? { hash: envHash, sha: null, source: 'env' } : null;
}
